...
pub struct ConsensusState {
    ...
    consensus_op_wal: Wal,
    soft_state: Option<SoftState>,
    toc: Arc<TableOfContent>,
    on_consensus_op_apply:
        HashMap<ConsensusOperations, oneshot::Sender<Result<bool, StorageError>>>,
...
impl ConsensusState {
    pub fn new(
        ...
        let consensus_op_wal = {
            let collections_meta_wal_path =
                Path::new(toc.storage_path()).join(&COLLECTIONS_META_WAL_DIR);
    ...
    fn wal_entry(&self, id: u64) -> raft::Result<RaftEntry> {
        ...
        let offset = first_entry.index - self.consensus_op_wal.first_index();
    ...
    fn wal_entries(
        ...
        (low..high)
            ...
            .map(|id| self.wal_entry(id))
            .collect()
    }

    pub fn first_wal_entry(&self) -> Result<Option<RaftEntry>, StorageError> {
        let first_index = self.consensus_op_wal.first_index();
        let entry = self
            .consensus_op_wal
            .entry(first_index)
    ...
    pub fn last_wal_entry(&self) -> Result<Option<RaftEntry>, StorageError> {
        let last_index = self.consensus_op_wal.last_index();
        let entry = self
            .consensus_op_wal
            .entry(last_index)
            .map(|entry| <RaftEntry as prost::Message>::decode(entry.as_ref()));
    ...
    pub fn set_raft_soft_state(&mut self, state: &SoftState) {
        self.soft_state = Some(SoftState { ..*state });
    }

    pub fn this_peer_id(&self) -> PeerId {
        self.persistent.this_peer_id
    }

    pub async fn cluster_status(&self) -> ClusterStatus {
        let hard_state = &self.persistent.state.hard_state;
        let peers = self
            .persistent
            .peer_address_by_id()
            .into_iter()
            ...
            .collect();
        let pending_operations = self.persistent.unapplied_entities_count();
        ClusterStatus::Enabled(ClusterInfo {
            ...
            raft_info: RaftInfo {
                ...
                pending_operations,
                leader: self.soft_state.as_ref().map(|state| state.leader_id),
                role: self
                    .soft_state
                    .as_ref()
                    .map(|state| state.raft_state.into()),
            },
        })
    }
    ...
    pub fn apply_entries<T: Storage>(
        ...
        loop {
            let unapplied_index = self.persistent.current_unapplied_entry();
            let entry_index = match unapplied_index {
                Some(index) => index,
                None => break,
            };
            log::info!("Applying committed entry with index {entry_index}");
            let entry = self.wal_entry(entry_index)?;
            ...
            self.persistent.entry_applied().map_err(raft_error_other)?;
        }
        Ok(())
    }

    pub fn apply_normal_entry(&mut self, entry: &RaftEntry) -> Result<bool, StorageError> {
        let operation: ConsensusOperations = entry.try_into()?;
        let on_apply = self.on_consensus_op_apply.remove(&operation);
    ...
    pub fn append_entries(&mut self, entries: Vec<RaftEntry>) -> Result<(), StorageError> {
        ...
        for entry in entries {
            ...
            self.consensus_op_wal.append(&buf)?;
        }
        Ok(())
    }
    ...
    pub fn apply_snapshot(
        ...
        self.consensus_op_wal.clear()?;
        self.persistent.apply_state_update(move |state| {
        ...
        self.persistent.set_peer_address_by_id(data.address_by_id)?;
        self.toc.apply_collections_snapshot(data.collections_data)?;
        Ok(())
    }

    pub fn set_hard_state(
        &mut self,
    ...
    pub fn hard_state(&self) -> raft::eraftpb::HardState {
        self.persistent.state().hard_state.clone()
    }

    pub fn set_commit_index(&mut self, index: u64) -> Result<(), StorageError> {
        self.persistent
            .apply_state_update(|state| state.hard_state.commit = index)
    }

    pub fn add_peer(&mut self, peer_id: PeerId, uri: Uri) -> Result<(), StorageError> {
        self.persistent.insert_peer(peer_id, uri)
    }

    // FIXME: this might deadlock as propose takes mut self and then it awaits completion - but for completion this state needs to change!
    // move it out into a separate fn which lock state only in beginning
    pub async fn propose_consensus_op(
        ...
        self.on_consensus_op_apply.insert(operation, sender);
        self.propose_sender.send(serialized)?;
    ...
    pub fn is_new_deployment(&self) -> bool {
        self.persistent.is_new()
    }

    pub fn peer_address_by_id(&self) -> PeerAddressById {
        self.persistent.peer_address_by_id()
    }
}

impl Storage for ConsensusState {
    fn initial_state(&self) -> raft::Result<RaftState> {
        Ok(self.persistent.state.clone())
    }
    ...
    fn entries(
        ...
        self.wal_entries(low, high, max_size)
    }

    fn term(&self, idx: u64) -> raft::Result<u64> {
        let raft_state = self.persistent.state();
        if idx == raft_state.hard_state.commit {
            return Ok(raft_state.hard_state.term);
        }
        Ok(self.wal_entry(idx)?.term)
    }

    fn first_index(&self) -> raft::Result<u64> {
        let index = match self.first_wal_entry().map_err(raft_error_other)? {
            Some(entry) => entry.index,
            None => self.persistent.state().hard_state.commit + 1,
        };
        Ok(index)
    }

    fn last_index(&self) -> raft::Result<u64> {
        let index = match self.last_wal_entry().map_err(raft_error_other)? {
            Some(entry) => entry.index,
            None => self.persistent.state().hard_state.commit,
        };
        Ok(index)
    }
    ...
    fn snapshot(&self, request_index: u64) -> raft::Result<raft::eraftpb::Snapshot> {
        ...
        let raft_state = self.persistent.state().clone();
        if raft_state.hard_state.commit >= request_index {
            let snapshot = SnapshotData {
                collections_data,
                address_by_id: self.persistent.peer_address_by_id(),
            };
...
#[derive(Clone)]
pub struct ConsensusStateRef(pub Arc<Mutex<ConsensusState>>);
...
impl Storage for ConsensusStateRef {
    ...
    fn entries(
        ...
        self.0.lock().entries(low, high, max_size)
    }

    fn term(&self, idx: u64) -> raft::Result<u64> {
        self.0.lock().term(idx)
    }

    fn first_index(&self) -> raft::Result<u64> {
        self.0.lock().first_index()
    }
    ...
    fn snapshot(&self, request_index: u64) -> raft::Result<raft::eraftpb::Snapshot> {
        self.0.lock().snapshot(request_index)
    }
}
...
impl Persistent {
    ...
    fn init(path: PathBuf, first_peer: bool) -> Result<Self, StorageError> {
        ...
        let learners = if true == first_peer {
            vec![this_peer_id]
        } else {
