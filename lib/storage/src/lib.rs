...
use content_manager::{
    collection_meta_ops::CollectionMetaOperations,
    consensus_ops::ConsensusOperations,
    consensus_state::{ConsensusState, ConsensusStateRef},
    errors::StorageError,
    toc::TableOfContent,
};
...
impl Dispatcher {
    ...
    pub async fn cluster_status(&self) -> ClusterStatus {
        match self.consensus_state.as_ref() {
            Some(state) => state.0.lock().cluster_status().await,
